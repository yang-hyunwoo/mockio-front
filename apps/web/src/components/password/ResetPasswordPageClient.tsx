...
export default function ResetPasswordPageClient() {
    ...
    useEffect(() => {
        const validateToken = async () => {
            if (!token) {
                setTokenError(
                    "유효하지 않은 접근입니다. 이메일의 재설정 링크를 다시 확인해 주세요."
                );
                setIsChecking(false);
                return;
            }
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setError(
                "비밀번호 변경에 실패했습니다. 링크를 다시 요청한 뒤 재시도해 주세요."
            );
        } finally {
                ...
                <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
                    <section className="rounded-3xl border border-white/10 bg-[var(--surface-glass-strong)] p-8 shadow-[0_24px_48px_rgba(20,30,50,0.12)] backdrop-blur">
                        ...
                        <p className="mt-4 max-w-xl text-sm leading-7 text-[var(--brand-muted)] sm:text-base">
                            이메일로 받은 재설정 링크를 통해 비밀번호를 변경할 수 있습니다. 링크는
                            일정 시간이 지나면 만료되며, 한 번 사용한 링크는 다시 사용할 수
                            없습니다.
                        </p>
                        ...
                        <div className="mt-8 grid gap-4 sm:grid-cols-2">
                            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                                ...
                                <p className="mt-2 text-sm leading-6 text-[var(--brand-muted)]">
                                    재설정 링크는 1회용으로 사용되며, 만료 시간이 지나면 다시 요청해야
                                    합니다.
                                </p>
                            </div>
                        ...
                        {isChecking ? (
                            <div className="flex min-h-[420px] flex-col justify-center">
                                <div className="rounded-3xl border border-sky-500/20 bg-sky-500/10 p-6">
                                    ...
                                    <p className="mt-3 text-sm leading-7 text-[var(--brand-muted)]">
                                        잠시만 기다려 주세요. 링크가 유효하면 바로 비밀번호를 변경할 수
                                        있습니다.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="flex min-h-[420px] flex-col justify-center">
                                <div className="rounded-3xl border border-emerald-500/20 bg-emerald-500/10 p-6">
                                    ...
                                    <p className="mt-3 text-sm leading-7 text-[var(--brand-muted)]">
                                        비밀번호가 정상적으로 변경되었습니다. 보안을 위해 기존 로그인
                                        세션이 종료될 수 있으며, 다시 로그인해 주세요.
                                    </p>
                                </div>
                            ...
                            <>
                                <div>
                                    ...
                                    <p className="mt-3 text-sm leading-6 text-[var(--brand-muted)]">
                                        새로운 비밀번호를 입력해 주세요. 이전에 사용한 비밀번호와 다른
                                        값으로 설정하는 것을 권장합니다.
                                    </p>
                                </div>
